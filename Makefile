...
# Add the regex library to the linker flags
LDFLAGS += -lregex

# If using PCRE, use the following instead:
# LDFLAGS += -lpcre
...
# Ensure LDFLAGS is used in the linking step
.PHONY:     scipbinary
scipbinary: $(SOPLEXDIR) $(ZIMPLDIR) $(SCIPDIR)
	@$(MAKE) -f Makefile.doit SCIPDIR=$(SCIPDIR) SOPLEXDIR=$(SOPLEXDIR) ZIMPL=$(ZIMPL) PAPILO=$(PAPILO) ZIMPLDIR=$(ZIMPLDIR) PAPILODIR=$(PAPILODIR) SPX_LEGACY=$(SPX_LEGACY) LDFLAGS="$(LDFLAGS)"
